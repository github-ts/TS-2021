...
/* Query DB */
$test = mysqli_query($con, "SELECT * FROM `prescn` WHERE ip='$ip'"); // test, if ip is already given
if(mysqli_num_rows($test) != 1) {mysqli_query($con, "INSERT INTO `prescn` SET ip='$ip'");} // else, make db entry
$count1 = mysqli_query($con, "SELECT cid FROM `prescn`"); // select, all ids in db
